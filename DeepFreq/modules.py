...
class FrequencyCountingModule(nn.Module):
    def __init__(self, n_output, n_layers, n_filters, kernel_size, fr_size, downsampling, kernel_in):
        ...
        if torch.__version__ >= "1.7.0":
            mod = [nn.Conv1d(1, n_filters, kernel_in, stride=downsampling, padding=(kernel_in - downsampling)//2, padding_mode='circular')] 
            #Not sure if this will work all the time just verified it for the 50 sample case
            ...
            for i in range(n_layers):
                mod += [
                    nn.Conv1d(n_filters, n_filters, kernel_size=kernel_size, padding=kernel_size - 1, bias=False, padding_mode='circular'),
                    nn.BatchNorm1d(n_filters),
                    nn.ReLU(),
                ]

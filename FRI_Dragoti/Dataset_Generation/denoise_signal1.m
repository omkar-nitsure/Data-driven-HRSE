function [s_m_denoised] = denoise_signal1(s_m_org, K, enh_method)
% -------------------------------------------------------------------------
...
switch lower(enh_method)
    case "cadzow"
        ...
        while more
            %%% SVD
            for start_idx = 1:block_size:P-K
                end_idx = min(start_idx + block_size - 1, P+1);
                if end_idx - start_idx + 1 < K+1
                    break;
                end
                A0 = toeplitz(s_m(K+start_idx:end_idx), s_m(K+1:-1:start_idx));
                [~, S0, ~] = svd(A0, 'econ');

                %%% CADZOW ENHANCEMENT
                A = toeplitz(s_m(J+start_idx:end_idx), s_m(J+1:-1:start_idx));
                [U, S, V] = svd(A, 'econ');
                S = diag(S);
                S((K + 1):end) = 0;
                S = diag(S);
                ...
                for p = start_idx:end_idx
                    s_m(p) = mean(diag(A, J - p));
                end
            end
            iter = iter + 1;
            if maxIter == Inf
               more = (K + 1 <= size(S0, 1)) && (S0(K + 1, K + 1) / norm(s_m) > 1e-7);
            else
               more = (iter < maxIter);
            end
        end
...
%s_m_denoised(isnan(s_m_denoised)) = 0; % or any other value you deem appropriate
s_m_denoised(isnan(s_m_denoised)) = s_m_org(isnan(s_m_denoised)); 
end

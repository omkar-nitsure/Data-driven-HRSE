...
out_filename = "deltat%d_%sdB_new.h5";
num_data = 10;                 % Number of data
PSNR_range = 70:-5:70;          % Select PSNR
...
t_k_array(1,:) = t_k0;
t_k_array(2,:) = 0.1+(1/60);%+(1/60);%t_k0 +logspace(-2.8,-3,1);
...
for j = 1:size(t_k_array,2)
    ...
    for k = 1:length(PSNR_range)
        ...
        s_m_a = zeros(num_data, 60);

        for i =1:num_data
            inamp = (0.5+9.5*rand)*ones(K,1);  % Amplitudes are equal but uniformly distributed between 0.5 and 10
            [t_k(i,:),a_k(i,:),y_n(i,:),s_m_a(i,:)] = generate_data_single143(N,K,T,T_s,phi,samp_mode,PERIODIC,inamp,inloc);
        end
        t_k = t_k(1,:);
        t_k_array(:,j) = t_k;
